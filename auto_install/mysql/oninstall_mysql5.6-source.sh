...
info_echo "Installing MySQL from source"
MYSRC="mysql-5.6.22-linux-glibc2.5-x86_64"
MYSRCTAR="mysql-5.6.22-linux-glibc2.5-x86_64.tar.gz"
#(1)judge soft
if [ ! -f $MYSRCTAR ]; then
  err_echo "[ERROR]: $MYSRCTAR is not exist, pls check it."
fi

#(2)untar soft
mkdir -p $INSTALL_PREFIX
tar zxf $MYSRCTAR -C $INSTALL_PREFIX
ln -s $INSTALL_PREFIX/$MYSRC $BASE_PREFIX
yum install libaio -y

#(3)create mysql user
groupadd mysql
useradd -g mysql -M mysql
sed -i 's#/usr/local/mysql#'$BASE_PREFIX'#g' $BASE_PREFIX/bin/mysqld_safe
...
do
	...
	mkdir -p $LOD_PREFIX/mysql${port}/slowlogs
        ...
        $BASE_PREFIX/scripts/mysql_install_db --basedir=$BASE_PREFIX --defaults-file=$MYDATA_PREFIX/mysql${port}/my.cnf --user=mysql --explicit_defaults_for_timestamp >/dev/null 
        if [ $? -ne 0 ]; then
	...
	/sbin/chkconfig add mysql${port}
        ...
        /usr/sbin/lsof -i :${port} &>/dev/null && $BASE_PREFIX/bin/mysqladmin -u root password "$ROOTPWD" -S $MYSQL_SOCK >/dev/null 2>&1
        if [ $? -ne 0 ]; then
            warn_echo "Failed to set MySQL root password! Please do it manually."
            break
        fi

        $BASE_PREFIX/bin/mysql -uroot -p$ROOTPWD -S $MYSQL_SOCK -Bse "SELECT concat(version(),' started in ${port} port')" 2>/dev/null
        ...
        $BASE_PREFIX/bin/mysql -uroot -p$ROOTPWD -S $MYSQL_SOCK -Bse "GRANT PROCESS,REPLICATION CLIENT ON *.* TO 'monitoring'@'127.0.0.1 identified by '8e3d7855e5681ee463e28394c2bb33043e65dbb9';FLUSH PRIVILEGES;" 2>/dev/null
		
	#(7)mysql privileges cfg
	info_echo "Install rpl_semi_sync on $port .... "
        $BASE_PREFIX/bin/mysql -uroot -p$ROOTPWD -S $MYSQL_SOCK -e "INSTALL PLUGIN rpl_semi_sync_master SONAME 'semisync_master.so';INSTALL PLUGIN rpl_semi_sync_slave SONAME 'semisync_slave.so';" 2>/dev/null
        if [ $? -ne 0 ]; then
