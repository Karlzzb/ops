...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COMMENT='后台管理员帐号信息表';
...
CREATE TABLE `vbos_bill_stat` (
  ...
  PRIMARY KEY (`auid`),
  KEY `ctime_key` (`ctime`)
) ENGINE=InnoDB AUTO_INCREMENT=61 DEFAULT CHARSET=utf8 COMMENT='每日对充值、金额的统计存储表';
...
CREATE TABLE `vbos_channel` (
  ...
  `uid` int(11) NOT NULL COMMENT '推广人 ID',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=latin1 COMMENT='推广渠道表';
...
CREATE TABLE `vbos_channel_stat` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '非业务主键ID',
  `name` varchar(255) CHARACTER SET utf8mb4 NOT NULL COMMENT '渠道名称',
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=latin1 COMMENT='推广统计表';
...
CREATE TABLE `vbos_host_info` (
  ...
  `host_type` varchar(128) NOT NULL DEFAULT '0' COMMENT '主播类型',
  ...
  `birtyday` datetime DEFAULT NULL COMMENT '生日',
  ...
  `editor` int(11) DEFAULT '0' COMMENT '编辑者ID',
  `ctime` datetime NOT NULL COMMENT '统计的时间',
  `dml_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据最后修改时间',
  `dml_flag` tinyint(4) NOT NULL DEFAULT '1' COMMENT '数据操作标识: 1-新增;2-修改;3-删除',
  PRIMARY KEY (`uid`),
...
CREATE TABLE `vbos_host_stat` (
  ...
  `username` varchar(128) NOT NULL COMMENT '主播账号',
  PRIMARY KEY (`host_id`,`ctime`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='每日对主播业绩的统计表';

/*Table structure for table `vbos_host_stat_copy` */

DROP TABLE IF EXISTS `vbos_host_stat_copy`;

CREATE TABLE `vbos_host_stat_copy` (
  `host_id` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '主播ID，和主播表的主键关联',
  `host_shares` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '当日的主播提成比例',
  `total_income` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '当日的总收入',
  `host_income` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '当日主播的收入',
...
CREATE TABLE `vbos_mrecharge_stat` (
  ...
  `content` text COMMENT '充值原因',
  `ctime` datetime NOT NULL COMMENT '统计的时间',
  `status` int(8) NOT NULL COMMENT '1=增加成功，-1=接口连接失败 -102=增加失败，-103=来源ip没有权限，-104=用户不存在',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=40 DEFAULT CHARSET=utf8 COMMENT='手工充值流程统计表';
...
CREATE TABLE `vbos_personal_channel_stat` (
  ...
  `nickname` varchar(64) COLLATE utf8_unicode_ci NOT NULL COMMENT '用户昵称',
  `regtime` datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '注册时间',
  `url` varchar(128) COLLATE utf8_unicode_ci NOT NULL,
  `invite_last_num` int(11) NOT NULL COMMENT '邀请昨日总量',
  `invite_num` int(8) NOT NULL COMMENT '邀请账号数',
  `reward` int(11) NOT NULL COMMENT '获得奖励',
  `created` datetime NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uid` (`uid`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='个人推广数据统计表';

/*Table structure for table `vbos_priv` */
...
CREATE TABLE `vbos_priv` (
  ...
  PRIMARY KEY (`priv_id`)
) ENGINE=InnoDB AUTO_INCREMENT=43 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='权限表';

insert  into `vbos_priv`(`priv_id`,`priv_name`,`parent`) values (1,'运营管理',0),(2,'财务管理',0),(3,'主播管理',0),(4,'会员管理',0),(5,'内容管理',0),(6,'广告管理',0),(7,'权限管理',0),(8,'即时数据',1),(9,'用户数据',1),(10,'营收数据',1),(11,'道具销售',1),(12,'节目点播',1),(13,'主播业绩',1),(14,'充值明细',2),(15,' 提款审批',2),(16,'提款记录',2),(17,' 手动充值',2),(18,' 主播审核',3),(19,'主播列表',3),(20,'主播详情',3),(21,'分成机制',3),(22,'会员列表',4),(23,'重置密码',4),(24,'封停账号',4),(25,'会员详情',4),(26,'客服消息',4),(27,'投诉建议',4),(28,'内容分类',5),(29,'道具管理',5),(30,'场景管理',5),(31,'广告位管理',5),(32,'广告配置',6),(33,'推广渠道',6),(34,'广告统计',6),(35,'费用添加',6),(36,'渠道管理',6),(37,'个人推广',6),(38,'后台用户',7),(39,'权限设定',7),(40,'道具销售日志',1);

/*Table structure for table `vbos_priv_user` */

DROP TABLE IF EXISTS `vbos_priv_user`;

CREATE TABLE `vbos_priv_user` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '非业务主键ID',
  `priv_id` int(11) NOT NULL COMMENT '权限ID',
  `admin_id` int(11) NOT NULL COMMENT '用户ID',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=959 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='权限用户关联表';
...
CREATE TABLE `vbos_timing_task` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '非业务主键ID',
  `conditions` text CHARACTER SET utf8 NOT NULL COMMENT '插入查询条件',
  `status` tinyint(2) NOT NULL DEFAULT '0' COMMENT '是否已执行   0：未执行   1：已执行',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1 COMMENT='系统消息发送TASK 任务记录表                      **创建者：morgan';
...
CREATE TABLE `vbos_usr_stat` (
  ...
  PRIMARY KEY (`auid`),
  KEY `ctime_key` (`ctime`)
) ENGINE=InnoDB AUTO_INCREMENT=60 DEFAULT CHARSET=utf8 COMMENT='用户访问统计表';
