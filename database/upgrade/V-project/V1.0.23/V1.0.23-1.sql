...
1.video-admin_v1.0.23_20150720_C.sql
*/
use video_bos;
CREATE TABLE `vbos_images_text` (
  `img_text_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '图片ID',
  `name` varchar(128) COLLATE utf8_bin NOT NULL COMMENT '图片名称',
  `temp_name` varchar(128) COLLATE utf8_bin NOT NULL DEFAULT '0' COMMENT '存储名称（哈希）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='图文表';

CREATE TABLE `video_game_operator` (
  `auto_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
  `uid` int(11) NOT NULL DEFAULT '0' COMMENT '运营ID',
  `points` int(11) NOT NULL DEFAULT '0' COMMENT '游戏获得总收入,正数赢钱，负数输钱',
  `type` int(11) NOT NULL DEFAULT '1' COMMENT '游戏类型：1赛车',
  `dml_flag` tinyint(1) NOT NULL DEFAULT '1' COMMENT '数据操作标识: 1-新增;2-修改;3-删除',
  `init_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间',
  `dml_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据最后修改时间',
  PRIMARY KEY (`auto_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='游戏运营员收入表';
...
/*
/script/svn/trunk/video-admin/V1.0.23/sql
2.video-admin_v1.0.23_20150720_D.sql
*/
use video_bos;

/*****************************后台表*********************************/
DROP TABLE IF EXISTS `video_game_operator`;
...
/*
/script/svn/trunk/video-admin/V1.0.23/sql
3.video-admin_v1.0.23_20150720_F.sql
*/
use video_bos;
alter table vbos_user_mexp add `curr_exp` int(11) NOT NULL DEFAULT '0' COMMENT '当前经验';

/*
/script/svn/trunk/video-front/V1.0.23/sql
1.video-front_v1.0.23_20150720_D.sql
*/
use video;
DROP TABLE IF EXISTS `video_game_operator`;
CREATE TABLE `video_game_operator` (
...
INSERT INTO `video_game_operator` VALUES ('1', '1264196', '0', '1', '1', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
INSERT INTO `video_game_operator` VALUES ('2', '1264202', '0', '1', '1', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);

/*
/script/svn/trunk/socket/V1.0.23/sql
1.V23_mysql.sql
*/
use video;
CREATE TABLE `video_cargame_bet` (
   `auto_id` int(10) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
   `gameid` int(10) DEFAULT NULL COMMENT '赛车游戏ID',
   `uid` int(10) DEFAULT NULL COMMENT '投注账号',
   `rid` int(10) DEFAULT '0' COMMENT '房间Id',
   `points` int(10) DEFAULT '0' COMMENT '投注钻石数',
   `carnum` smallint(1) DEFAULT '0' COMMENT '投注赛车号码', 
	 `win` int(10) DEFAULT '0' COMMENT '玩家纯收入,正数为赢钱（除抽水）,负数为输钱',
   `draw` int(10) DEFAULT '0' COMMENT '玩家赢钱抽水',
   `created` datetime DEFAULT NULL COMMENT '创建时间',
   `dml_flag` tinyint(1) NOT NULL DEFAULT '1' COMMENT '数据操作标识: 1-新增;2-修改;3-删除', 
	 `init_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间', 
	 `dml_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据最后修改时间', 
    PRIMARY KEY (`auto_id`),
    KEY `index_cargame_uid` (`uid`) USING BTREE,
    KEY `index_cargame_gameId` (`gameId`) USING BTREE,
    KEY `index_cargame_created` (`created`) USING BTREE
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='当前赛车游戏投注信息';
 

CREATE TABLE `video_cargame_bet_bak` (
   `auto_id` int(10) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
   `gameid` int(10) DEFAULT NULL COMMENT '赛车游戏ID',
   `uid` int(10) DEFAULT NULL COMMENT '投注账号',
   `rid` int(10) DEFAULT '0' COMMENT '房间Id',
   ...
   `created` datetime DEFAULT NULL COMMENT '创建时间',
    ...
    KEY `index_cargame_gameId` (`gameId`) USING BTREE,
    KEY `index_cargame_created` (`created`) USING BTREE
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='历史赛车游戏投注信息';

CREATE TABLE `video_cargame` (
   `gameid` int(10)  NOT NULL AUTO_INCREMENT COMMENT '游戏id',
   `stime` datetime DEFAULT NULL COMMENT '游戏开始时间',
   `etime` datetime DEFAULT NULL COMMENT '游戏结束时间',
   `carnum` smallint(1) DEFAULT '0' COMMENT '开奖号码', 
 	 `uid` int(10) DEFAULT NULL COMMENT '庄家账号',
 	 `rid` int(10) DEFAULT '0' COMMENT '所在房间Id',
   `points` int(10) DEFAULT '0' COMMENT '庄家投注钻石数',
   `times` smallint(2) DEFAULT '0' COMMENT '抢庄倍数', 
   `win` int(10) DEFAULT '0' COMMENT '庄家纯收入,正数为赢钱（除抽水）,负数为输钱',
   `draw` int(10) DEFAULT '0' COMMENT '庄家赢钱抽水',
   `totaluser` int(10) DEFAULT '0' COMMENT '押注总人数',
   `totalpoints` int(10) DEFAULT '0' COMMENT '押注总金额',
   `totaldraw` int(10) DEFAULT '0' COMMENT '总赢钱抽水',
   `rate` smallint(3) DEFAULT '0' COMMENT '赢钱抽水比例0-100', 
   `channel` smallint(3) DEFAULT '0' COMMENT '渠道编号id', 
   `dml_flag` tinyint(1) NOT NULL DEFAULT '1' COMMENT '数据操作标识: 1-新增;2-修改;3-删除', 
	 `init_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间', 
	 `dml_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据最后修改时间', 
    PRIMARY KEY (`gameId`),
    KEY `index_cargame_uid` (`uid`) USING BTREE
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='赛车游戏';

CREATE TABLE `video_cargame_settlement` (
   `auto_id` int(10) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
   `rid` int(10) DEFAULT '0' COMMENT '房间Id',
   `totalbanker` int(10) DEFAULT '0' COMMENT '庄家人数',
   `totaluser` int(10) DEFAULT '0' COMMENT '押注总人数',
   `totalpoints` int(10) DEFAULT '0' COMMENT '押注总金额',
   `totaldraw` int(10) DEFAULT '0' COMMENT '总赢钱抽水',
   `created` date NOT NULL DEFAULT '1970-01-01' COMMENT '统计日期',
	 `dml_flag` tinyint(1) NOT NULL DEFAULT '1' COMMENT '数据操作标识: 1-新增;2-修改;3-删除', 
	 `init_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间', 
	 `dml_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据最后修改时间', 
    PRIMARY KEY (`auto_id`),
    KEY `index_cargame_settlement_rid` (`rid`) USING BTREE
 ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='赛车游戏统计信息';
