...
error(){
  ...
  /usr/bin/zabbix_sender -z 172.16.100.14 -p 10051 -s TPHN-PAY-backup-check -k mysql.backup  -o 172.16.100.14-zabbix > /dev/null 2>&1
  exit 1
}
...
MYSQL_SECURITIES="--socket=/data/db/mysql3306/mysql3306.sock --user=${USERNAME} --password=${PASSWORD}"
...
BACK_DIR=/backup/zabbix-dbbackup
