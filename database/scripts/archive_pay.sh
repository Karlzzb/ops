...
MYSQL_USER="root"
MYSQL_PASS="a099e03ef05102d4cf4e8e7ab565af61"
MYSQL_SOCK="/data/db/mysql3398/mysql3398.sock"
MYSQL_BIN="/opt/mysql/bin/mysql -u $MYSQL_USER -p$MYSQL_PASS --socket=$MYSQL_SOCK"
...
ARCHIVER_CONF="/home/dbtools/archive_pay.conf"

#error header
error(){
  echo "`date +%F_%T` [ERROR]  $1" >> $ERR_LOG
  /usr/bin/zabbix_sender -z 172.16.100.14 -p 10051 -s caegi-backup-check -k mysql.data_archiver  -o "archive failed" > /dev/null 2>&1
  final_deal
  exit 1
}
...
data_archive() {
  ...
  $ARCHIVER --source ${ARCHIVER_CONNS},D=${TABLE_SCHEMA},t=${TABLE_NAME} --dest ${ARCHIVER_CONNS},D=archive_db,t=${TABLE_NAME}${ARCHIVE_SUFFIX} --no-check-charset --where "$ARCHIVE_WHERE"  --limit 1000 --low-priority-insert --bulk-insert --ignore --low-priority-delete --commit-each  --statistics --why-quit --local --progress 100000 >> $ERR_LOG 2>&1 || error "$TABLE_SCHEMA.$TABLE_NAME archive failed"
}
...
/usr/bin/zabbix_sender -z 172.16.100.14 -p 10051 -s caegi-backup-check -k mysql.data_archiver  -o OK > /dev/null 2>&1
echo "" > $ERR_LOG
