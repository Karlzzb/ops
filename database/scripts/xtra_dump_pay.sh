...
error(){
  ...
  /usr/bin/zabbix_sender -z 172.16.100.14 -p 10051 -s TPHN-PAY-backup-check -k mysql.backup  -o 172.16.100.4 > /dev/null 2>&1
  exit 1
}
...
USERNAME="dumpusr"
PASSWORD=""
MYSQL_CNF=/data/db/mysql3398/my.cnf
MYSQL_SECURITIES="--host=${HOSTNAME} --port=${PORT} --user=${USERNAME} --password=${PASSWORD}"
...
#Limitations
FULL_BACKUP_GAP=259200 # Lifetime of the latest full backup in seconds
KEEP=3 # Number of full backups to keep
STARTED_AT=`date +%s`
...
/usr/bin/zabbix_sender -z 172.16.100.14 -p 10051 -s TPHN-PAY-backup-check -k mysql.backup  -o OK > /dev/null 2>&1
